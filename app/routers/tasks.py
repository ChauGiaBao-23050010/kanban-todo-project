...
def get_tasks(
    ...
    tasks = task_repository.get_by_board(db, board_id) # Lấy tất cả task của board trước
...
def delete_task(task_id: int, db: Session = Depends(get_db)):
    ...
    return {"message": "Xóa task thành công", "deleted_task_id": task_id}
...
def search_tasks(
    ...
    matching_tasks = task_repository.search_tasks(db, q, board_id)
    return [TaskResponse.from_orm(task) for task in matching_tasks]